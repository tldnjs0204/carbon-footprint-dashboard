...
with col_time1:
    travel_time_min = st.slider("이동 시간을 선택하세요 (분)", 10, 480, 60, step=10)
with col_time2:
    st.metric("선택한 시간", f"{travel_time_min}분", f"= {travel_time_min/60:.1f}시간")
...
for idx, (col, mode, color) in enumerate(zip([col_tree1, col_tree2, col_tree3, col_tree4], modes_list, colors)):
    trees_needed = distance_emission[mode] / tree_absorption_per_year
    col.metric(f"🌲 {mode}", f"{trees_needed:.1f}그루
필요", help="1년간 흡수할 나무 수")
...
with tab2:
    ...
    if distance_km > 0:
        ...
        for mode in ['버스', '철도', '항공기']:
            ...
            with col_alt1:
                st.metric(f"배출량 감소", f"{reduction:.2f} kg CO2", f"{reduction_pct:.1f}% 절감")
            with col_alt2:
                trees = reduction / tree_absorption_per_year
                st.metric(f"나무 절약", f"{trees:.1f}그루")
            st.markdown("---")
