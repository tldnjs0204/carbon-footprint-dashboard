...
reduction = total_emission_kg - (train_row["1시간당 배출량(g)"] * time_hours * frequency_per_trip / 1000)
...
if selected_transport in ["고속열차 (KTX/SRT)", "지하철 / 전기열차"]:
    ...
    st.plotly_chart(px.pie(values=[total_emission_kg, total_emission_kg - reduction], names=["현재", "전환"], color_discrete_sequence=["#F2C4B1", "#A3C9AE"], hole=0.4), use_container_width=True)
