...
# -----------------------------------------------------------------------------
# 1. 페이지 설정 및 커스텀 스타일 (참고 이미지 기반 차분한 세이지 그린/버터 옐로우/소프트 주황 테마)
# -----------------------------------------------------------------------------
st.set_page_config(
...
# [스타일 커스텀] 참고 사진의 부드러운 그린(#48825D, #8FB89A), 버터 옐로우(#E4EEB0), 은은한 소프트 살몬 주황(#FFB899) 테마
st.markdown("""
<style>
...
# -----------------------------------------------------------------------------
# 5. 연간 탄소 허용량 대비 소진율 원형 그래프 (참고 이미지 색상 반영)
# -----------------------------------------------------------------------------
...
with col_pie_left:
    ...
    # [차트 1 색상] 은은한 소프트 살몬 주황(#FFB899) & 참고 사진의 딥 세이지 그린(#48825D) 조합
    fig_budget_pie = px.pie(
        pie_data,
        names="구분",
        values="배출량(kg)",
        color="구분",
        color_discrete_sequence=["#FFB899", "#48825D"],
        hole=0.4
    )
...
# -----------------------------------------------------------------------------
# 6. 교통수단별 온실가스 배출량 비교 시각화 (참고 이미지 기반 색상 팔레트)
# -----------------------------------------------------------------------------
...
with col_left:
    ...
    # 참고 이미지의 차분한 세이지 그린, 버터 옐로우, 은은한 살몬 주황 적용
    pastel_color_map = {
        "대중교통": "#8FB89A",   # 부드러운 민트 세이지 그린
        "개인교통": "#E4EEB0",   # 사진 속 버튼의 버터 옐로우/연두
        "항공": "#FFB899"        # 은은하고 부드러운 소프트 주황
    }
...
with col_right:
    ...
    # [차트 2 색상] 은은한 소프트 살몬 주황(#FFB899) & 부드러운 세이지 그린(#8FB89A) 조합
    fig_pie = px.pie(
        compare_df, 
        names="구분", 
        values="총 배출량(kg)",
        color="구분",
        color_discrete_sequence=["#FFB899", "#8FB89A"],
        hole=0.4
    )
