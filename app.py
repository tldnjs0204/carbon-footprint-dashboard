...
with col_pie_left:
    ...
    # [차트 1 색상 수정] 파스텔 로즈 핑크(#FF758F) & 파스텔 소프트 민트(#88D49E) 조합
    fig_budget_pie = px.pie(
        pie_data,
        names="구분",
        values="배출량(kg)",
        color="구분",
        color_discrete_sequence=["#FF758F", "#88D49E"],
        hole=0.4
    )
...
with col_right:
    ...
    # [차트 2 색상 수정] 파스텔 탠저린 오렌지(#FFB347) & 파스텔 스카이 블루(#4EA8DE) 조합
    fig_pie = px.pie(
        compare_df, 
        names="구분", 
        values="총 배출량(kg)",
        color="구분",
        color_discrete_sequence=["#FFB347", "#4EA8DE"],
        hole=0.4
    )
